<template>
    <div class="nav-container">
      <div class="header" style="display: flex; justify-content: space-between;">
        <el-button type="primary" icon="el-icon-plus" @click="showAddDialog">添加类别</el-button>
        <el-button type="success" icon="el-icon-check" @click="openLinkInNewTab">跳转导航页</el-button>
      </div>
        ...
        <div v-for="item in navList" :key="item.id" class="nav-item">
          <div class="nav-header">
            <div style="display: flex; align-items: center; gap: 10px;">
              <span>{{ item.name }}</span>
              <el-button size="mini" type="success" @click="showAddLinkDialog(item.id)">添加链接</el-button>
          ...
          <div class="link-list" v-if="item.links && item.links.length">
            <div v-for="link in item.links" :key="link.id" class="link-item">
              <div class="link-content">
                ...
                <div class="link-text">
                  <div class="link-name" style="font-size: 16px; font-weight: bold; color: #333;">{{ link.name }}</div>
                  <div  style="font-size: 13px; color: #666; margin-top: 4px;">{{ link.desc }}</div>
                  <a :href="link.url" target="_blank" class="link-url" style="font-size: 13px; color: #409EFF; margin-top: 4px;">{{ link.url }}</a>
                </div>
              </div>
      ...
      <el-dialog :title="dialogTitle" :visible.sync="dialogVisible" width="30%">
        <el-form :model="form" :rules="rules" ref="form">
          ...
          <el-form-item label="排序" prop="nav_sort">
            ...
            <span style="margin-left: 10px; color: #999; font-size: 12px;">数字越大排序越靠前</span>
          </el-form-item>
        </el-form>
...
import { Checkbox, Icon } from 'element-ui'
  ...
  export default {
    ...
    methods: {
      ...
      showAddLinkDialog(navId) {
        ...
        this.linkForm = { name: '', desc: '', url: '',   navId, id: null }
