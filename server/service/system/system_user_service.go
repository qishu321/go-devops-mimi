...
func (s UserService) List(req *systemReq.UserListReq) ([]*system.User, error) {
	...
	db := common.DB.Model(&system.User{}).Order("id DESC")
...
func (s UserService) ListAll() (list []*system.User, err error) {
	err = common.DB.Model(&system.User{}).Order("created_at DESC").Find(&list).Error
...
func (s UserService) Count() (int64, error) {
	var count int64
	err := common.DB.Model(&system.User{}).Count(&count).Error
	return count, err
}

// Exist 判断资源是否存在
func (s UserService) Exist(filter map[string]interface{}) bool {
	var dataObj system.User
...
func (s UserService) ChangeStatus(id, status int) error {
	return common.DB.Model(&system.User{}).Where("id = ?", id).Update("status", status).Error
}
