...
func (s ApiService) List(req *systemReq.ApiListReq) ([]*system.Api, error) {
	var list []*system.Api
	db := common.DB.Model(&system.Api{}).Order("created_at DESC")
...
func (s ApiService) ListAll() (list []*system.Api, err error) {
	err = common.DB.Model(&system.Api{}).Order("created_at DESC").Find(&list).Error
...
func (s ApiService) Count() (int64, error) {
	var count int64
	err := common.DB.Model(&system.Api{}).Count(&count).Error
	return count, err
}
...
func (s ApiService) Delete(ids []uint) error {
	...
	err := common.DB.Where("id IN (?)", ids).Unscoped().Delete(&system.Api{}).Error
